...
void LLScriptLibrary::init()
{
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "llGetTimeOfDay", "f", "", "float llGetTimeOfDay()\ngets the time in seconds since Meerkat server midnight (or since server up-time; whichever is smaller)"));
...
// script calls for vehicle action
	...
	// new LSL calls in OpenSim, from MEER-118 -Patrick Sapinski (Wednesday, August 19, 2009)
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osTerrainSetHeight", NULL, "iif", "integer osTerrainSetHeight(int x, int y, double val)\nsets terrain height X & Y Values, double = float\nuse osTerrainsFlush() afterwards\nMust be Server Enabled"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetRot", NULL, "kq", "osSetRot(UUID target, Quaternion rotation)\nrotates target by Rot Vector<X,Y,Z>\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetDynamicTextureURL", NULL, "ssssi", "string osSetDynamicTextureURL(string dynamicID, string contentType, string url, string extraParams, int timer )\nSets textures from website\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetDynamicTextureURLBlend", NULL, "ssssii", "string osSetDynamicTextureURLBlend(string dynamicID, string contentType, string url, string extraParams, int timer, int alpha)\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetDynamicTextureURLBlendFace", NULL, "ssssfiiii", "string osSetDynamicTextureURLBlendFace(string dynamicID, string contentType, string url, string extraParams, bool blend, int disp, int timer, int alpha, int face)\nLoads a web texture on a prim.\nShould work for any image URL (including php scripts) that Opensim can render.\n[Note refreshTimer may not yet be implemented, use a timer event and recall the function to get the same effect]\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetDynamicTextureData", NULL, "ssssi", "string osSetDynamicTextureData(string dynamicID, string contentType, string data, string extraParams, int timer)\nWrites text and vector graphics onto a prim face.\nwidth,height and alpha value can be passed\ne.g. osSetDynamicTextureData('', 'vector', drawList, 'width:64,height:1024:alpha:0', 0)\n! REPLACE Single Quotes with Double Quotes\nTo turn off the alpha layer use 'alpha:false' in the optional parameters\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetDynamicTextureDataBlend", NULL, "ssssii", "string osSetDynamicTextureDataBlend(string dynamicID, string contentType, string data, string extraParams, int timer, int alpha)\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetDynamicTextureDataBlendFace", NULL, "ssssfiiii", "string osSetDynamicTextureDataBlendFace(string dynamicID, string contentType, string data, string extraParams, bool blend, int disp, int timer, int alpha, int face)\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osConsoleCommand", NULL, "s", "osConsoleCommand(string command)\nIssues Commands directly to the OpenSim Server Console\nMust be Enabled on Server (SEVERE RISK)"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osMovePen", NULL, "sii", "string osMovePen(string drawList, int x, int y)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawLine", NULL, "siiii", "string osDrawLine(string drawList, int startX, int startY, int endX, int endY)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawLine", NULL, "sii", "string osDrawLine(string drawList, int endX, int endY)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawText", NULL, "ss", "string osDrawText(string drawList, string text)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawEllipse", NULL, "sii", "string osDrawEllipse(string drawList, int width, int height)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawRectangle", NULL, "sii", "string osDrawRectangle(string drawList, int width, int height)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawFilledRectangle", NULL, "sii", "string osDrawFilledRectangle(string drawList, int width, int height)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetFontSize", NULL, "si", "string osSetFontSize(string drawList, int fontSize)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetPenSize", NULL, "si", "string osSetPenSize(string drawList, int penSize)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetPenColour", NULL, "ss", "string osSetPenColour(string drawList, string colour)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osDrawImage", NULL, "siis", "string osDrawImage(string drawList, int width, int height, string imageUrl)\nRefer to: http://opensimulator.org/wiki/Dynamic_textures\nMust be Server Enabled"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetRegionWaterHeight", NULL, "f", "osSetRegionWaterHeight(double height)\nAdjusts Water Height on region\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetRegionSunSettings", NULL, "iif", "osSetRegionSunSettings(bool useEstateSun, bool sunFixed, double sunHour)\nWhere bool = 0 or 1, double = float\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSetEstateSunSettings", NULL, "if", "osSetEstateSunSettings(bool sunFixed, double sunHour)\nWhere bool = 0 or 1, double = float\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetCurrentSunHour", "f", NULL, "double osGetCurrentSunHour()\nReturns Float Value of Current Sun Hour\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSunGetParam","f", "s", "double osSunGetParam(string param)\nReturns current float values for param\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osSunSetParam", "sf", NULL, "osSunSetParam(string param, double value)\nSet Param for SunSet, double = float\nMust be Server Enabled"));
	...
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetNotecardLine", "s", "si", "string osGetNotecardLine(string name, int line)Returns line data from named Notecard\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetNotecard", "s", "s", "string osGetNotecard(string name)\nReturns contents of Notecard into string\nMust be Server Enabled"));
	addFunction(new LLScriptLibraryFunction(10.f, 0.f, dummy_func, "osGetNumberOfNotecardLines", "i", "s", "int osGetNumberOfNotecardLines(string name)\nReturns total number of lines in a notecard\nMust be Server Enabled"));
