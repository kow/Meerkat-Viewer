...
export SL_CMD='$LL_WRAPPER bin/do-not-directly-run-meerkat-bin'
...
if [ -n "$LL_RUN_ERR" ]; then
	...
	if [ "$LL_RUN_ERR" = "runerr" ]; then
		...
		if [ "`uname -m`" = "x86_64" ]; then
			echo
			cat << EOFMARKER
You are running the Meerkat Viewer on a x86_64 platform.  The
...
echo
echo '*******************************************************'
echo 'This is a BETA release of the Meerkat linux client.'
