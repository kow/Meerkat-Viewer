...
// 	U32 cache_max_entries = LLAppViewer::getTextureCache()->getMaxEntries();
	...
	std::string text;
	text = llformat("GL Tot: %d/%d MB Bound: %d/%d MB Raw Tot: %d MB Bias: %.2f",
					total_mem,
					max_total_mem,
					bound_mem,
					max_bound_mem,
	...
	left = bar_left;
	text = "GL";
	LLFontGL::sMonospace->renderUTF8(text, 0, left, line_height*3,
											 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	...
	LLFontGL::sMonospace->renderUTF8(text, 0, left, line_height*2,
											 color, LLFontGL::LEFT, LLFontGL::TOP);
...
void LLGLTexSizeBar::draw()
{
	...
	if(LLImageGL::sCurTexSizeBar == mIndex)
	{
		...
		LLFontGL::sMonospace->renderUTF8(text, 0, mLeft, mBottom + (S32)(mTopLoaded * mScale) + mLineHeight,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);

		text = llformat("%d", mTopBound) ;
		LLFontGL::sMonospace->renderUTF8(text, 0, (mLeft + mRight) / 2, mBottom + (S32)(mTopBound * mScale) + mLineHeight,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	}
...
{	
	...
	//x axis: size label
	text = llformat("%d", 0) ;
	LLFontGL::sMonospace->renderUTF8(text, 0, left + 12, bottom - line_height / 2,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	for(U32 i = 1 ; i < mTextureSizeBar.size() ; i++)
	{
		text = llformat("%d", (1 << (i / 2)) + ((i & 1) ? ((1 << (i / 2)) >> 1) : 0)) ;
		LLFontGL::sMonospace->renderUTF8(text, 0, left + i * mTextureSizeBarWidth + 12, bottom - line_height / 2,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	}
	text = llformat("(w + h)/2") ;
	LLFontGL::sMonospace->renderUTF8(text, 0, right + 10, bottom - line_height / 2,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	//-------
	...
	for(S32 i = bottom + 50 ; i <= top ; i += 50)
	{
		...
		LLFontGL::sMonospace->renderUTF8(text, 0, left - 20, i + line_height / 2 ,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
		LLFontGL::sMonospace->renderUTF8(text, 0, right + 5, i + line_height / 2 ,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	}
	...
	LLFontGL::sMonospace->renderUTF8(text, 0, left + 100, top + line_height * 2,
									 loaded_color, LLFontGL::LEFT, LLFontGL::TOP);

	F32 bound_color[] = {1.0f, 1.0f, 0.0f, 0.75f};
	gl_rect_2d(left + 170, top + line_height * 2, left + 190, top + line_height, bound_color) ;
	text = llformat("Bound") ;
	LLFontGL::sMonospace->renderUTF8(text, 0, left + 200, top + line_height * 2,
	...
	LLFontGL::sMonospace->renderUTF8(text, 0, left + 250, top + line_height * 3,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	return scale ;
}
...
F32 LLTextureSizeView::drawTextureCategoryDistributionGraph() 
{
	...
	text = llformat("MB") ;
	LLFontGL::sMonospace->renderUTF8(text, 0, left - 20, top + line_height * 2 ,
									 text_color, LLFontGL::LEFT, LLFontGL::TOP);
	//--------------------------------------------------
	F32 loaded_color[] = {1.0f, 0.0f, 0.0f, 0.75f};
	gl_rect_2d(left + 70, top + line_height * 2, left + 90, top + line_height, loaded_color) ;
