...
{
	bool mature_enabled = TRUE; //gAgent.canAccessMature();
	bool adult_enabled = TRUE; //gAgent.canAccessAdult();
								 ...
								 LLUUID* id)
{
	...
	*hit_type = 0; // hit nothing
	...
	if (level <= DRAW_SIMINFO_THRESHOLD)
	{
		bool show_mature = TRUE; //gAgent.canAccessMature() && gSavedSettings.getBOOL("ShowMatureEvents");
		bool show_adult = TRUE; //gAgent.canAccessAdult() && gSavedSettings.getBOOL("ShowAdultEvents");
