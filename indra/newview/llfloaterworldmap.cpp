...
void LLFloaterWorldMap::draw()
{
	...
	childSetVisible("events_mature_icon", true);
	childSetVisible("events_mature_label", true);
	childSetVisible("event_mature_chk", true);
...
void LLFloaterWorldMap::onSelectServer(LLUICtrl* ctrl, void* userdata)
{
	...
	LLTracker::stopTracking((void *)(intptr_t)TRUE);
	//LLTracker::stopTracking(NULL);
//	LLWorldMap::getInstance()->mIsTrackingLocation = FALSE; This probably needs fixing -Patrick Sapinski (Wednesday, October 21, 2009)
	self->mSLURL = "";				// Clear the SLURL since it's invalid
	self->mSetToUserPosition = TRUE;	// Revert back to the current user position
