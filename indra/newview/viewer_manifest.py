...
class ViewerManifest(LLManifest):
    ...
    def channel_unique(self):
        return self.channel().replace("Meerkat", "").strip()
...
#                  dst="win_crash_logger.exe")
    ...
    def package_finish(self):
        ...
        if self.default_channel():
            if self.default_grid():
                # release viewer
                installer_file = "Meerkat_%(version_dashes)s_Setup.exe"
                grid_vars_template = """
                OutFile "%(installer_file)s"
                !define INSTFLAGS "%(flags)s"
                !define INSTNAME   "Meerkat"
                !define SHORTCUT   "Meerkat"
                !define URLNAME   "secondlife"
                Caption "Meerkat ${VERSION}"
                """
            else:
                # beta grid viewer
                installer_file = "Meerkat_%(version_dashes)s_(%(grid_caps)s)_Setup.exe"
                grid_vars_template = """
                OutFile "%(installer_file)s"
                !define INSTFLAGS "%(flags)s"
                !define INSTNAME   "Meerkat%(grid_caps)s"
                !define SHORTCUT   "Meerkat (%(grid_caps)s)"
                !define URLNAME   "secondlife%(grid)s"
                !define UNINSTALL_SETTINGS 1
                Caption "Meerkat %(grid)s ${VERSION}"
                """
        else:
            # some other channel on some grid
            installer_file = "Meerkat_%(version_dashes)s_%(channel_oneword)s_Setup.exe"
            grid_vars_template = """
            OutFile "%(installer_file)s"
            !define INSTFLAGS "%(flags)s"
...
class DarwinManifest(ViewerManifest):
    def construct(self):
        ...
        self.path(self.args['configuration'] + "/Meerkat.app", dst="")
        ...
        if self.prefix(src="", dst="Contents"):  # everything goes in Contents
            ...
            self.path("../../libraries/universal-darwin/lib_release/libalut.dylib", dst="MacOS/libalut.dylib")

            # replace the default theme with our custom theme (so scrollbars work).
            if self.prefix(src="app_settings/mozilla/chrome", dst="MacOS/chrome"):
...
#                self.path("../mac_crash_logger/" + self.args['configuration'] + "/mac-crash-logger.app", "mac-crash-logger.app")
    ...
    def package_finish(self):
        channel_standin = 'Meerkat'  # hah, our default channel is not usable on its own
        ...
        if self.default_channel() and not self.default_grid():
            app_name = "Meerkat " + self.args['grid']
        else:
...
class Linux_i686Manifest(LinuxManifest):
    def construct(self):
        ...
        self.path("secondlife-stripped","bin/do-not-directly-run-meerkat-bin")
...
#            self.path("libkdu_v42R.so")
            ...
            self.path("libopenal.so.0")
            self.path("libopenal.so.0.0.0")
