		...
		if( mIsSelf )
		{
			...
			if ( lastGesticulationLevel != mCurrentGesticulationLevel )
			{
				if ( mCurrentGesticulationLevel != VOICE_GESTICULATION_LEVEL_OFF )
				{
					...
					LLGestureManager::getInstance()->triggerAndReviseString( gestureString );
				}
			}
...
//		if (sRenderGroupTitles != mRenderGroupTitles)
		...
		// If alpha > 0, create mNameText if necessary, otherwise delete it
		{
			...
			if (alpha > 0.f)
			{
				...
				if(!gSavedSettings.getBOOL("MeerkatChangeColorOnClient"))
				{
					avatar_name_color = gColors.getColor( "AvatarNameColor" );				
				}
				if(!gSavedSettings.getBOOL("MeerkatClientTagDisplay"))
				{
					client = "";
				}
