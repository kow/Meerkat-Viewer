...
int main(int argc, char **argv)
{
	...
	if(!gCancelled && (gFailure != noErr))
	{
		...
		err = CreateStandardAlert(
				kAlertStopAlert,
				CFSTR("Error"),
				CFSTR("An error occurred while updating Meerkat.  Please download the latest version from www.secondlife.com."),
				&params,
				&alert);
...
void filterFile(const char* filename)
{
	...
	snprintf(temp, sizeof(temp), 		
			"sed 's/Meerkat/%s/g' '%s.tmp' > '%s'", gProductName, filename, filename);
	system(temp);		/* Flawfinder: ignore */
}
